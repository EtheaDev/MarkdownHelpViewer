{******************************************************************************}
{                                                                              }
{       MarkDown Help Viewer: View Image resources Unit                        }
...
{                                                                              }
{******************************************************************************}
unit MDHelpView.ImageCollection;
...
type
  TdmImageCollection = class(TDataModule)
...
procedure TdmImageCollection.DataModuleCreate(Sender: TObject);
begin
  FStream := TMemoryStream.Create;
end;

procedure TdmImageCollection.DataModuleDestroy(Sender: TObject);
begin
  FreeAndNil(FStream);
  inherited;
end;

procedure TdmImageCollection.HtmlViewerImageRequest(Sender: TObject;
...
procedure TdmImageCollection.StopLoadingImages(const AStop: Boolean);
begin
  FStopImageRequest := AStop;
end;

function TdmImageCollection.getStreamData(const AFileName : String;
  const AMaxWidth: Integer; const ABackgroundColor: TColor): TStream;
var
  sl        : TStringList;
  bFail     : Boolean;
...
procedure TdmImageCollection.ConvertImage(AFileName: string;
  const AMaxWidth: Integer; const ABackgroundColor: TColor);
var
  LPngImage: TPngImage;
  LBitmap: TBitmap;
...
begin
  ...
  try
    ...
    begin
      ...
      begin
        LBitmap := TBitmap.Create;
        LBitmap.SetSize(Round(LSVG.Width), Round(LSVG.Height));
      end;
      try
        ...
        LSVG.PaintTo(LBitmap.Canvas.Handle, LRect, True);
        FStream.Clear;
